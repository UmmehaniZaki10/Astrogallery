...
def test_nft_creation(contracts, accounts, chain):
    ...
    def test_user_flow_pre_staking():
        assert staking_contract.calculateUserReward(user_1).return_value == 0
        assert staking_contract.totalStakedAmount() == 0
        assert staking_contract.claimableTokens(user_1).return_value == 0
    ...
    def test_rewards(lock_up_period,amount_to_stake):
        ...
        chain.snapshot()
        assert (
            staking_contract.calculateUserReward(user_1).return_value == 0
        ), "Wrong user reward"
        chain.sleep(int(lock_up_period/2) * ONE_DAY)
        assert isclose(
            staking_contract.calculateUserReward(user_1).return_value,
    ...
    def test_claimable_tokens(amount_to_stake):
        ...
        chain.snapshot()
        assert (
            staking_contract.claimableTokens(user_1).return_value == 0
        ...
        chain.mine(2)
        assert (
            staking_contract.claimableTokens(user_1).return_value == amount_to_stake
        ), "Incorrect claimable tokens"
    ...
    def test_withdraw_flow(lock_up_period):
        ...
        claimable_tokens = staking_contract.claimableTokens(user_1).return_value 
